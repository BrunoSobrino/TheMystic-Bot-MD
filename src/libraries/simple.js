  ...
  value(text = "") {
    try {
      ...
      const esNumeroValido = (numero) => {
        // Eliminamos restricciones de longitud específicas
        ...
        console.log(`[DEBUG] Validación número ${numero}: ${valido ? 'VÁLIDO' : 'inválido'}`);
        return valido;
      };
        ...
        .map((numero) => {
          if (esNumeroValido(numero)) {
            ...
            if (!global.lidResolver?.lidCache) {
              console.log('[DEBUG] lidCache no disponible');
              return `${numero}@lid`;
            }

            // Búsqueda optimizada en el cache
            const cacheEntries = Array.from(global.lidResolver.lidCache.entries());
            const cachedJid = cacheEntries.find(([key]) => 
              key.startsWith(`${numero}@lid_`) || 
              key === `${numero}@lid`
            )?.[1];

            if (cachedJid) {
              console.log(`[DEBUG] LID resuelto: ${numero} -> ${cachedJid}`);
              return cachedJid;
            }

            console.log(`[DEBUG] LID no encontrado en caché. Entradas disponibles:`, 
              cacheEntries.slice(0, 3).map(([k]) => k));
            
            return `${numero}@lid`;
          }
        });

      console.log('[DEBUG] Resultado final:', mentions);
      return mentions;
        
    } catch (error) {
      console.error('[ERROR] En parseMention:', error, error.stack);
      return [];
    }
  },
  enumerable: true,
