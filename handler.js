 ...
 *[ ℹ️ ] Si consideras que esto es un error y cuentas con pruebas, puedes comunicarte con el propietario(a) del bot para apelar la suspensión.*`.trim();
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          mconn.conn.reply(m.chat, `*[ 💱 ] لقد نفد الماس الخاص بك، يمكنك شراء المزيد باستخدام الأمر:* _${usedPrefix}شراءالكل`, m);
          continue; 
        }
        if (plugin.level > _user.level) {
          mconn.conn.reply(m.chat, `*[ 💱 ] ويشترط أن يكون المستوى ${plugin.level} لتتمكن من استخدام الأمر.  مستواك الحالي هو ${_user.level}, استخدم الأمر ${usedPrefix}لفل لرفع مستواك مع XP.*`, m);
          continue; 
        }
        const extra = {
          match,
        ...
        try {
          ...
          if (m.limit) {
            m.reply('*[ 💱 ] تم استخدامها ' + +m.limit + ' الماس (الماس) (حدود).*');
          }
        }
        break;
      }
      ...
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          ...
          try {
            ...
            const botTt2 = groupMetadata.participants.find((u) => m.conn.decodeJid(u.id) == m.conn.user.jid) || {};
            const isBotAdminNn = botTt2?.admin === 'admin' || false;
            text = (action === 'add' ? (chat.sWelcome || this.welcome || conn.welcome || 'نورت يحب, @user!').replace('@subject', await m.conn.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝚂𝙸𝙽 𝙳𝙴𝚂𝙲𝚁𝙸𝙿𝙲𝙸𝙾𝙽*') :
                              (chat.sBye || this.bye || conn.bye || 'خرج وريحنا, @user!')).replace('@user', '@' + user.split('@')[0]);
            if (userPrefix && chat.antiArab && botTt.restrict && isBotAdminNn && action === 'add') {
              const responseb = await m.conn.groupParticipantsUpdate(id, [user], 'remove');
              if (responseb[0].status === '404') return;
              const fkontak2 = {'key': {'participants': '0@s.whatsapp.net', 'remoteJid': 'status@broadcast', 'fromMe': false, 'id': 'Halo'}, 'message': {'contactMessage': {'vcard': `BEGIN:VCARD\nVERSION:3.0\nN:Sy;Bot;;;\nFN:y\nitem1.TEL;waid=${user.split('@')[0]}:${user.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}, 'participant': '0@s.whatsapp.net'};
...
let date = d.toLocaleDateString('es', { day: 'numeric', month: 'long', year: 'numeric' })
 let time = d.toLocaleString('en-US', { hour: 'numeric', minute: 'numeric', second: 'numeric', hour12: true })
	...
	if (!chat?.antidelete) return 
        if (!msg) return 
	if (!msg?.isGroup) return 
	const antideleteMessage = `_*< الرسائل المحذوفه />*_\n
 ▢ *المستخدم:* @${participant.split`@`[0]}
 ▢ *الوقت:* ${time}
 ▢ *تاريخ:* ${date}\n
 ▢ *جاري إرسال الرسالة المحذوفة...*\n
 *[ 💠 ] لإلغاء تنشيط الوظيفة* _antidelete_*, أرسل الأمر التالي:* _/تعطيل مانع الحذف_`.trim();
        await mconn.conn.sendMessage(msg.chat, {text: antideleteMessage, mentions: [participant]}, {quoted: msg})
        mconn.conn.copyNForward(msg.chat, msg).catch(e => console.log(e, msg))
    } catch (e) {
        console.error(e)
