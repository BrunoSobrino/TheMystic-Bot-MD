...
*👉يمكنك الاتصال بمالك البوت إذا كنت تعتقد أنه خطأ (لديك دليل) للتعامل مع سبب الحظر الخاص بك*
👉 201554824764
`.trim();
...
if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
  ...
  user.commandCount = 1
        ...
        if (!m.fromMem && m.text.match(/Natsu|mahmoud|@201554824764|@201004779310|محمود|ناتسو|بوت|bot|natsu|Mahmoud|حوده/gi)) {
        let emot = pickRandom(["🎃", "❤", "😘", "😍", "💕", "😎", "🙌", "⭐", "👻", "🔥","🥹","🫶","✨","🌹","👑","💕"]) 
        this.sendMessage(m.chat, { react: { text: emot, key: m.key }})}
        function pickRandom(list) { return list[Math.floor(Math.random() * list.length)]}

      if (!m.fromMem && m.text.match(/اوامر|menu/gi)) {
        let emot = pickRandom(["📃"]) 
        this.sendMessage(m.chat, { react: { text: emot, key: m.key }})}
...
export async function callUpdate(callUpdate) {
    ...
    await this.sendMessage(nk.from, { contacts: { displayName: '𝑁𝐴𝑇𝑺𝑈 👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] تنبيه - إشعار', body: '𝑁𝐴𝑇𝑺𝑈 - ʙᴏᴛ', thumbnail: imagen1, sourceUrl: 'tiktok.com/@dead.1000' }}}}, aa)
