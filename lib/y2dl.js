...
let bestFormat = (url, type) => new Promise(async (resolve, reject) => {
    let at = await fetch("https://srvcdn8.2convert.me/api/json?url=" + url, {
        method: "GET",
        headers: {
    ...
    let json = await at.json()
    if (json.error == true) reject(false)
    let formats = json.formats
    if (type == "audio") {
        let formatau = formats.audio
    ...
    if (type == "video") {
        ...
        let format = formats.video[formats.video.length - 1]
        return resolve(format)
    }
})

let getUrlDl = (url) => new Promise(async (resolve, reject) => {
    let taskid = await fetch("https://srvcdn8.2convert.me/api/json", {
        method: "POST",
        headers: {
    ...
    while (status == false) {
        await sleep(1000)
        let statusid = await fetch("https://srvcdn8.2convert.me/api/json/task", {
            method: "POST",
            headers: {
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
                "origin": "https://en1.y2mate.is",
