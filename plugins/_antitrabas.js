...
export async function before(m, { conn, isAdmin, isBotAdmin, usedPrefix }) {
  ...
  if (chat.antiTraba && m.text.length > 5000) { // Cantidad máxima de caracteres aceptados en un mensaje.
    if (isAdmin) return conn.sendMessage(m.chat, { text: `${tradutor.texto1} @${await m.sender.split('@')[0]} ${tradutor.texto1_1}`, mentions: [await m.sender] }, { quoted: fakemek });
    ...
    if (isBotAdmin && bot.restrict) {
      ...
      setTimeout(() => {
        conn.sendMessage(m.chat, { text: `${tradutor.texto3} \n${'\n'.repeat(400)}\n=> ${tradutor.texto3_1} wa.me/${await m.sender.split('@')[0]}\n=> ${tradutor.texto3_2} ${name}\n${tradutor.texto3_3}`, mentions: [await m.sender] }, { quoted: fakemek });
      }, 0);
      setTimeout(() => {
        conn.groupParticipantsUpdate(m.chat, [await m.sender], 'remove');
      }, 1000);
