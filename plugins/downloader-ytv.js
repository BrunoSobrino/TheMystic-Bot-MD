...
const handler = async (m, {conn, args, usedPrefix, command}) => {
  ...
  try {
   ...
   if (fileSizeInMB > 200) {
    ...
    await conn.sendMessage(m.chat, {text: `*[ ❌ ] El video no pudo ser enviado debido a que pesa mas de 1GB, el video pesa ${fileSizeInMB} MB, puedes descargar el video en el siguiente link: ${formats.url}.*`, edit: key}, {quoted: m});
    enviando = false
   } else {
    await conn.sendMessage(m.chat, {video: buff, caption: `_𝐓𝐡𝐞 𝐌𝐲𝐬𝐭𝐢𝐜 - 𝐁𝐨𝐭_`, fileName: ttl_1 + '.mp4', mimetype: 'video/mp4'}, {quoted: m});
