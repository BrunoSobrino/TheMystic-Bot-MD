...
*「 𝐔𝐒𝐔𝐀𝐑𝐈𝐎𝐒 𝐏𝐑𝐄𝐌𝐈𝐔𝐌 」*${sortedP.slice(0, len).map(({ jid, name, premiumTime, prem, registered }, i) => {
    if (jid === userr) return '' // Si el jid del usuario actual es igual a userr, se omite la impresión
    return `
        —◉ Usuario: ${'@' + jid.split`@`[0]}
        ${premiumTime > 0 ? `*◉ Tiempo Premium:*\n${clockString (premiumTime - new Date() * 1)}` : '- Este es un usuario no premium'}
    `
}).filter(Boolean).join('')}`;
