...
const handler = async (m, {
    ...
    if (!text) throw `إعطاء نص للبحث مثال: *${usedPrefix + command}* أغنية moon light`;
    conn.GURUPLAY = conn.GURUPLAY ? conn.GURUPLAY : {};
    await conn.reply(m.chat, wait, m);
    const result = await searchAndDownloadMusic(text);
    const infoText = `✦ ──『 *𝛩𝐵𝐼𝑇𝛩* 』── ⚝ \n\n [ ⭐ قم بالرد على رقم نتيجة البحث المطلوبة للحصول على الصوت]. \n\n` ;
...
handler.before = async (m, {
    ...
    let title = generateRandomName();
        ...
        m.reply("رقم تسلسلي غير صالح. الرجاء تحديد الرقم المناسب من القائمة أعلاه.\nبين 1 إلى " + result.allLinks.length);
    }
};
