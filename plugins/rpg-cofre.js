...
const handler = async (m, {isPrems, conn}) => {
  ...
  const time = global.db.data.users[await m.sender].lastcofre + 86400000; // 36000000 10 Horas //86400000 24 Horas
  if (new Date - global.db.data.users[await m.sender].lastcofre < 86400000) throw `${tradutor.texto1[0]} *${msToTime(time - new Date())}* ${tradutor.texto1[1]}`;
  ...
  global.db.data.users[await m.sender].limit += dia;
  global.db.data.users[await m.sender].money += mystic;
  global.db.data.users[await m.sender].joincount += tok;
  global.db.data.users[await m.sender].exp += expp;
...
${tradutor.texto2[7]}`;
  ...
  global.db.data.users[await m.sender].lastcofre = new Date * 1;
};
handler.help = ['daily'];
handler.tags = ['xp'];
