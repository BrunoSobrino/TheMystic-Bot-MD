...
let res4 = res3.result
let { filename, filetype, filesize, uploaded, link } = await res4  
let caption2 = `
*📓 𝙽𝙾𝙼𝙱𝚁𝙴:* ${filename}
*📁 𝙿𝙴𝚂𝙾:* ${filesize}
*📄 𝚃𝙸𝙿𝙾:* ${filetype}
...
`.trim()
m.reply(caption2)
await conn.sendFile(m.chat, link, filename, null, m, null, { mimetype: filetype, asDocument: true })   
} catch {
try { 
...
`.trim()
m.reply(caption)
await conn.sendFile(m.chat, url2, filename, '', m, null, { mimetype: ext, asDocument: true })
} catch {  
