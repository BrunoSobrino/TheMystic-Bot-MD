...
export async function before(m, { conn, participants }) {
  ...
  try {     
    ...
    const mentionsString = [await m.sender, m.messageStubParameters[0], ...groupAdmins.map((v) => v.id)];
    const mentionsContentM = [await m.sender, m.messageStubParameters[0]];
      ...
      switch (m.messageStubType) {
        ...
        case 27: // Member add
          await safeOperation(async () => {
            ...
            if (!await m.sender.endsWith('@g.us')) {
              txt += `${tradutor.member_add.added_user.replace('@user', `@${m.messageStubParameters[0].split('@')[0]}`)}\n${tradutor.member_add.added_by.replace('@user', `@${await m.sender.split('@')[0]}`)}`;
            } else {
        ...
        case 32: // Member leave or remove
          await safeOperation(async () => {
            const ax = m.messageStubParameters[0] === await m.sender ? 'self_removed' : 'removed_user';
            let txt = `${tradutor.member_remove.header}\n\n${tradutor.member_remove.group.replace('@group', groupName)}\n`;
            if (ax === 'removed_user') {
              txt += `${tradutor.member_remove.removed_user.replace('@user', `@${m.messageStubParameters[0].split('@')[0]}`)}\n${tradutor.member_remove.removed_by.replace('@user', `@${await m.sender.split('@')[0]}`)}`;
            } else {
