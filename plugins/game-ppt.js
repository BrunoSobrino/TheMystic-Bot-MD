...
// let pp = 'https://www.bighero6challenge.com/images/thumbs/Piedra,-papel-o-tijera-0003318_1584.jpeg'
  ...
  const time = global.db.data.users[await m.sender].wait + 10000;
  if (new Date - global.db.data.users[await m.sender].wait < 10000) throw `${tradutor.texto1} ${Math.floor((time - new Date()) / 1000)} ${tradutor.texto2}`;
  ...
  if (textm == astro) {
    global.db.data.users[await m.sender].exp += 500;
    m.reply(`${tradutor.texto4[0]} ${textm}*\n${tradutor.texto4[1]} ${astro}*\n${tradutor.texto4[2]}`);
  } else if (text == 'papel') {
    if (astro == 'piedra') {
      global.db.data.users[await m.sender].exp += 1000;
      m.reply(`${tradutor.texto5[0]} ${textm}*\n${tradutor.texto5[1]} ${astro}*\n${tradutor.texto5[2]}`);
    } else {
      global.db.data.users[await m.sender].exp -= 300;
  ...
  global.db.data.users[await m.sender].wait = new Date * 1;
};
handler.help = ['ppt'];
handler.tags = ['games'];
