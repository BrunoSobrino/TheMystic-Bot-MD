...
['𝙼𝙰𝚃𝙴𝚂 𝙷𝙰𝚁𝙳', `${usedPrefix + command} hard`]], m)*/
  ...
  const id = m.chat;
  if (id in global.math) return conn.reply(m.chat, '*[ ℹ️ ] Aún quedan problemas matemáticos sin resolver en este chat.*', global.math[id][0]);
  const math = genMath(mode);
  global.math[id] = [
    await conn.reply(m.chat, `▢ *Resuelve:* ${math.str}\n\n▢ *Tiempo:* ${(math.time / 1000).toFixed(2)}s\n\n▢ *Premio:* ${math.bonus} XP`, m),
    math, 4,
    setTimeout(() => {
      if (global.math[id]) {
        conn.reply(m.chat, `*[ ℹ️ ] Se agotó el tiempo.*\n\n▢ *Respuesta:* ${math.result}`, m),
