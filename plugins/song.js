...
const handler = async (m, {command, usedPrefix, conn, text}) => {
  if (!text) throw `*[❗𝐈𝐍𝐅𝐎❗] ڪتب سمية ديال ديسڪ لي بيتي اصحبي*\n\n*—◉ example:*\n*${usedPrefix + command} morphine sucre sale*`;
  try {
    if (command == 'play') {
      conn.reply(m.chat, `*_⏳wa7d d9i9a ok...⏳_*`, m);
      try {
    ...
    if (command == 'play2') {
      conn.reply(m.chat, `*_wa7d d9i9a ok...⏳_*`, m);
      try {
        const mediaa = await ytPlayVid(text);
        const aa_2 = await conn.sendMessage(m.chat, {video: {url: mediaa.result}, fileName: `error.mp4`, caption: `_𝐓𝐡𝐞 prince lo9mane  ࣪ʕ•̫͡•ʔ_`, thumbnail: mediaa.thumb, mimetype: 'video/mp4'}, {quoted: m});
        if (!aa_2) {
        throw new Error('*[❗] El primero metodo fallo, intentando otro...*');
       }
      } catch {
        const res = await fetch(`https://api.lolhuman.xyz/api/ytplay2?apikey=${lolkeysapi}&query=${text}`);
        const json = await res.json();
        await conn.sendFile(m.chat, json.result.video, 'error.mp4', `_𝐓𝐡𝐞 prince lo9mane_`, m);
      }
    }
  } catch {
