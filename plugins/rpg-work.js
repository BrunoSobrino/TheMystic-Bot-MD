...
const handler = async (m, { conn, isPrems }) => {
  ...
  const time = global.db.data.users[await m.sender].lastwork + 600000;
  if (new Date - global.db.data.users[await m.sender].lastwork < 600000) throw `⚔️ *¡Espera un momento pequeño aventurero!* ⚔️\n\n*—◉ Regresa a la travesía en ${msToTime(time - new Date())} ⏳*`;
  ...
  global.db.data.users[await m.sender].exp += hasil;
  global.db.data.users[await m.sender].lastwork = new Date() * 1;
  enviando = false
};
handler.help = ['work'];
handler.tags = ['xp'];
