...
const handler = async (m, {text, conn, usedPrefix, command}) => {
  ...
  const why = `${tradutor.texto1} ${usedPrefix + command} @${await m.sender.split('@')[0]}*`;
  ...
  if (!who) conn.reply(m.chat, why, m, {mentions: [await m.sender]});
  const res = [];
  switch (command) {
    case 'blok': case 'block':
      ...
      } else conn.reply(m.chat, why, m, {mentions: [await m.sender]});
      break;
