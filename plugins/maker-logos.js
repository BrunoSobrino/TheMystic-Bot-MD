...
const handler = async (m, {conn, args: [effect], text: txt, usedPrefix, command, name}) => {
  ...
  const effectoSelect = effects.find((effectz) => effectz.title == effect);
  const res = await new Maker().TextPro(effectoSelect.url, ...text)
  await conn.sendMessage(m.chat, {image: {url: res.imageUrl}, caption: `*𝚃𝙾𝙼𝙰 𝚃𝚄 𝙸𝙼𝙰𝙶𝙴𝙽 𝙿𝙴𝚁𝚂𝙾𝙽𝙰𝙻𝙸𝚉𝙰𝙳𝙰!!*\n*𝙴𝙵𝙴𝙲𝚃𝙾: ${effect}*`}, {quoted: m})  
};
handler.help = ['logos'];
handler.tags = ['nulis'];
handler.command = /^(logo|logos|logo2)$/i;
